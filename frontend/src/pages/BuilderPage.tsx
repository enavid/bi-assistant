...
export function BuilderPage() {
  ...
  async function handleCreateSection() {
    ...
    const updated = await createSection.mutateAsync({ projectId: selected.id, section: { name: newSectionName.trim(), content: '', order } })
  ...
  async function handleOutputBlur(value: string) {
          ...
          <div className="flex-1 overflow-y-auto p-7">
            ...
            <div className="grid gap-3.5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))' }}>
              {(projects ?? []).map((p) => {
                ...
                return (
                  <div
                    ...
                    className="group bg-bg-surface border border-border-default rounded-xl p-4 cursor-pointer hover:border-accent-border transition-all hover:-translate-y-0.5"
                  >
              ...
              <div
                ...
                className="border border-dashed border-border-strong rounded-xl p-4 cursor-pointer flex flex-col items-center justify-center gap-2.5 min-h-[160px] hover:border-accent hover:bg-accent-bg transition-all"
              >
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <div className="h-[46px] border-b border-border-default flex items-center px-5 gap-3 flex-shrink-0 bg-bg-surface">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-text-2 hover:text-text-1 transition-colors"
          >
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Section list — FIX #6: 2px border separation */}
          <div className="w-[156px] min-w-[156px] border-r-2 border-border-default flex flex-col bg-bg-raised">
            ...
            <div className="flex-1 overflow-y-auto p-1.5">
              {(selected?.sections ?? []).map((s) => (
                <div
                  key={s.id}
                  onClick={() => { setActiveSectionId(s.id); setTab('sections') }}
                  className={clsx(
                    'group flex items-center gap-2 px-2.5 py-1.5 rounded-[7px] mb-1 cursor-pointer border transition-all',
            ...
            {/* FIX #8: prominent add section at bottom */}
            <div className="p-2 border-t border-border-subtle flex-shrink-0">
              <button
                onClick={() => setNewSectionOpen(true)}
                className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-[7px] border border-border-strong bg-bg-surface text-xs font-medium text-text-1 hover:border-accent transition-colors"
              >
          ...
          {/* Editor */}
          <div className="flex flex-col flex-1 overflow-hidden">
            {/* FIX #7: tab bar on raised bg with 2px bottom border */}
            <div className="flex border-b-2 border-border-default bg-bg-raised flex-shrink-0 px-3">
              {(['sections', 'output', 'notes', 'experiments'] as Tab[]).map((t, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setTab(t)}
                    className={clsx(
                      'flex items-center gap-1.5 px-3 py-2.5 text-xs transition-colors border-b-2 -mb-0.5',
                      tab === t ? 'text-text-1 border-accent font-medium' : 'text-text-3 border-transparent hover:text-text-2'
                    )}
                  >
            ...
            {tab === 'sections' && (
              <div className="flex-1 flex flex-col p-3.5 overflow-hidden gap-2">
                {activeSection ? (
                  <>
                    {/* FIX #3: section name only — no UUID in header */}
                    ...
                    />
                    {/* FIX #4: short hint with chip */}
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <span className="text-[11px] text-text-3">Use in Output Format as</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded-[20px] bg-accent-bg text-accent-text border border-accent-border font-mono">
                        {`{${activeSection.id}}`}
                      </span>
                    </div>
                  </>
            ...
            {tab === 'output' && selected && (
              <div className="flex-1 flex flex-col p-3.5 gap-3 overflow-hidden">
                <div className="flex-shrink-0">
                  <div className="text-[10px] font-medium text-text-3 uppercase tracking-[.6px] mb-2">Section references</div>
                  <div className="flex flex-wrap gap-1.5">
                    {selected.sections.map((s) => (
                      <span key={s.id} className="text-[11px] px-2 py-0.5 rounded-[20px] bg-bg-raised border border-border-strong text-text-2 font-mono">
                        <span className="text-text-1 font-medium">{`{${s.id}}`}</span> = {s.name}
                      </span>
                    ))}
                    <span className="text-[11px] px-2 py-0.5 rounded-[20px] bg-bg-raised border border-border-strong text-text-2 font-mono font-medium">{'{question}'}</span>
                  </div>
                </div>
                <textarea
                  key={selected.id + '-out'}
                  defaultValue={selected.output_format}
                  onBlur={(e) => handleOutputBlur(e.target.value)}
                  placeholder={'{section_id}\n\nQUESTION:\n{question}\n\nSQL:'}
                  className="flex-1 w-full bg-bg-raised border border-border-default rounded-[8px] p-3 text-xs font-mono text-text-1 leading-[1.75] resize-none outline-none focus:border-accent transition-colors"
                  style={{ direction: 'ltr', textAlign: 'left' }}
                />
                <div className="border-t border-border-subtle pt-3 flex-shrink-0 flex flex-col gap-2">
                  <div className="text-[10px] font-medium text-text-3 uppercase tracking-[.6px]">Test prompt</div>
                  ...
                  {testResult && (
                    <pre className="text-[11px] font-mono text-text-2 bg-bg-raised border border-border-default rounded-[7px] p-2.5 overflow-x-auto whitespace-pre-wrap">
                      {testResult}
                    </pre>
                  )}
                </div>
            ...
            {tab === 'notes' && selected && (
              <div className="flex-1 flex flex-col p-3.5 gap-2 overflow-hidden">
                ...
                />
                <span className="text-[11px] text-text-3 flex-shrink-0">Auto-saves on blur</span>
              </div>
            )}
              ...
              <div className="flex-1 overflow-y-auto p-3.5 flex flex-col gap-2">
                ...
                {selected.experiments.map((exp) => (
                  <div key={exp.id} className="bg-bg-raised border border-border-default rounded-[8px] p-3">
                    ...
                    <div className="text-[11px] text-text-2" style={{ direction: 'rtl', textAlign: 'right' }}>{exp.question}</div>
                    {exp.comment && <div className="text-[10px] text-text-3 mt-1">{exp.comment}</div>}
                  </div>
                ))}
              </div>
            )}
          ...
          {/* FIX #5: Preview — bigger font + line-height */}
          <div className="w-[210px] min-w-[210px] border-l-2 border-border-default flex flex-col bg-bg-raised">
            <div className="h-9 border-b border-border-subtle flex items-center px-3 flex-shrink-0">
              <span className="text-[10px] font-medium text-text-3 uppercase tracking-[.7px]">Assembled prompt</span>
            </div>
            <div className="flex-1 overflow-y-auto p-3">
              {preview ? (
                <pre className="text-[12px] font-mono text-text-2 leading-[1.65] whitespace-pre-wrap break-words">
                  {preview}
                </pre>
              ) : (
...
function buildPreview(project: Project | null): string {
  ...
  if (project.output_format) {
    ...
    for (const s of project.sections) result = result.replace(`{${s.id}}`, s.content || `[${s.name}]`)
    return result.replace('{question}', '[question]')
  }
  const parts = project.sections.slice().sort((a, b) => a.order - b.order).map((s) => s.content || `[${s.name}]`)
