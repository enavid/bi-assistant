...
class SemanticMapper:
    ...
    def map(self, question: str, context: Any | None = None, metadata: Any | None = None, **_: Any) -> JsonDict:
        ...
        mapped_concepts = self._cleanup_mapped_concepts_for_context(mapped_concepts, normalized_question)
        ...
        group_by = self._detect_group_by(normalized_question, semantic_layer, filters, mapped_concepts)
        query_features = self._detect_query_features(normalized_question)
        filters = self._cleanup_filters_for_context(filters, group_by, query_features, normalized_question)
        disambiguation_notes = self._apply_disambiguation(normalized_question, filters, group_by, mapped_concepts)
        ...
        for item in iterable:
            ...
            matches.append(
                TermMatch(
                    ...
                    metadata={k: v for k, v in item.items() if k not in {"term", "concept_id", "category", "route"}},
                )
            )
        return matches
    ...
    def _cleanup_mapped_concepts_for_context(mapped_concepts: list[JsonDict], normalized_question: str) -> list[JsonDict]:
        ...
        if "نوع استخدام" in normalized_question and not any(term in normalized_question for term in ["جذب", "سال جذب", "روند جذب", "اخیر"]):
            cleaned = [item for item in cleaned if item.get("concept_id") not in {"hiring", "hiring_trend_annual", "hiring_last_15_years"}]
    ...
    def _detect_value_filters(self, normalized_question: str, semantic_layer: JsonDict) -> list[JsonDict]:
        ...
        question_mentions_contract_type = any(term in normalized_question for term in ["نوع قرارداد", "قراردادهای", "قرارداد ", " قرارداد"])
        question_mentions_position_expert = any(term in normalized_question for term in ["پست کارشناسی", "نقش کارشناسی", "پست کارشناس", "عنوان پست کارشناسی"])
    ...
    def _detect_numeric_filters(self, normalized_question: str) -> list[JsonDict]:
        ...
        for pattern in [r"(?:زیر|کمتر از|پایین تر از)\s+(\d{1,3})\s*(?:سال)?", r"(\d{1,3})\s*سال\s*(?:به پایین|کمتر)"]:
            for match in re.finditer(pattern, normalized_question):
                ...
                if 0 < value < 100:
                    filters.append(self._numeric_filter("age", "<", value, match.group(0), f"v.age < {value}"))
        ...
        if any(term in normalized_question for term in ["بدون سابقه", "سابقه صفر", "صفر سابقه"]):
            filters.append(self._numeric_filter("service_years", "=", 0, "بدون سابقه", "COALESCE(v.service_years, 0) = 0"))
    ...
    def _detect_group_by(
        ...
        grouping_cues = ["به تفکیک", "بر اساس", "براساس", "در هر", "برای هر", "سهم هر", "توزیع", "نسبت", "روند"]
        ...
        if any(term in normalized_question for term in ["زن و مرد", "مرد و زن", "زنان و مردان", "تعداد زن و مرد"]):
            group_by.append(self._group_item("gender", "gender_pair_expression", ["زن و مرد"], 0.96))
        ...
        if "روند" in normalized_question and any(term in normalized_question for term in ["جذب", "استخدام"]):
            group_by.append(self._group_item("hire_year", "trend_hiring", ["روند جذب"], 0.95, preferred_visualization="line_chart"))

        if any(term in normalized_question for term in ["بیشترین جذب", "کمترین جذب", "سال بیشترین جذب", "سال کمترین جذب"]):
            group_by.append(self._group_item("hire_year", "most_or_least_hiring_year", ["بیشترین/کمترین جذب"], 0.93, preferred_visualization="table"))
            ...
            for concept in mapped_concepts:
                ...
                if column in dimension_columns and column not in filter_columns:
                    ...
                    if column == "hire_year" and not any(term in normalized_question for term in ["جذب", "سال جذب", "روند جذب", "استخدام سالانه"]):
                        continue
                    if column == "city":
                        group_by.append(self._group_item("city", "city_data_gap_dimension", concept.get("matched_terms", []), 0.89, sql="DATA_GAP"))
                    else:
                        group_by.append(self._group_item(str(column), "concept_dimension", concept.get("matched_terms", []), 0.84))
        ...
        implicit_group_patterns: list[tuple[list[str], str, str]] = [
            ...
            (["مدرک تحصیلی", "تحصیلات مختلف", "بر اساس مدرک"], "education_title", "implicit_education"),
            ...
            (["هر بخش", "هر واحد", "بخش", "واحد", "اداره", "دپارتمان"], "department_name", "implicit_department"),
    ...
    def _apply_disambiguation(
        ...
        if any(term in normalized_question for term in ["نوع استخدام", "استخدام"]):
            notes.append({"rule_id": "DISAMBIG_001", "applied": True, "mapping": "employment_type", "reason": "User mentioned نوع استخدام/استخدام."})
        if any(term in normalized_question for term in ["نوع قرارداد", "قرارداد"]):
            notes.append({"rule_id": "DISAMBIG_001", "applied": True, "mapping": "contract_type", "reason": "User mentioned نوع قرارداد/قرارداد."})

        if "پیمانکاری" in normalized_question:
            notes.append({"rule_id": "DISAMBIG_002", "applied": True, "mapping": "is_contractor = TRUE", "reason": "Contractor concepts use is_contractor for shares/counts."})
            ...
            if has_contractor_bool:
                filters[:] = [f for f in filters if not (f.get("column") == "employment_type" and f.get("value") == "شاغل در پیمانکاری")]

        if any(term in normalized_question for term in ["پست کارشناسی", "نقش کارشناسی", "پست کارشناس"]):
            notes.append({"rule_id": "DISAMBIG_003", "applied": True, "mapping": "is_expert_role", "reason": "کارشناسی appears in position context."})

        if any(term in normalized_question for term in ["شهر", "هر شهر", "تحلیل شهری"]):
            notes.append({"rule_id": "DISAMBIG_008", "applied": True, "mapping": "DATA_GAP", "reason": "City-level data is not reliable in MVP."})

        if any(term in normalized_question for term in ["آستانه بازنشستگی", "نزدیک بازنشستگی"]):
            notes.append({"rule_id": "DISAMBIG_007", "applied": True, "mapping": "DATA_GAP", "reason": "No formal retirement rule is defined."})
    ...
    def _rank_candidate_intents(
        ...
        *,
        ...
        def add_score(intent_id: str, score: float, reason: str) -> None:
            if not intent_id:
                return
            item = scores.setdefault(intent_id, {"intent_id": intent_id, "score": 0.0, "reasons": []})
            item["score"] += score
        ...
        for concept in mapped_concepts:
            ...
            for intent_id in maps_to.get("related_intents", []) or []:
                add_score(str(intent_id), 5.0 + float(concept.get("score", 0)) / 2, f"semantic:{concept.get('concept_id')}")
        ...
        for intent in intent_catalog.get("intents", []) or []:
            ...
            for term in intent.get("trigger_terms_fa", []) or []:
                normalized_term = self._normalize_term(term)
                if normalized_term and self._find_term(normalized_question, normalized_term) >= 0:
                    add_score(intent_id, 4.5 + min(len(normalized_term), 25) / 10, f"trigger:{term}")
            if self.config.include_example_overlap:
                for example in intent.get("user_examples", []) or []:
                    overlap = self._token_overlap(normalized_question, self._normalize_term(example))
                    if overlap >= 0.45:
    ...
    def _build_warnings(
        ...
        *,
        ...
        warnings: list[str] = []
        columns = {str(col.get("name")) for col in data_dictionary.get("columns", []) or [] if isinstance(col, dict)}
        missing = [col for col in mapped_columns if columns and col not in columns]
        if missing:
            warnings.append("Mapped columns not found in data_dictionary: " + ", ".join(sorted(set(missing))))
        if any(item.get("column") == "city" for item in group_by) or "شهر" in normalized_question:
            warnings.append("City-level questions should be handled as DATA_GAP in the current MVP.")
        if any(item.get("column") == "department_approved_headcount" for item in filters) or any("چارت مصوب" in str(item) for item in disambiguation_notes):
            warnings.append("For approved headcount, do not SUM department_approved_headcount over employee rows.")
...
if __name__ == "__main__":  # pragma: no cover - local smoke test.
    ...
    for q in sample_questions:
        result = mapper.map(q)
        print("\nQUESTION:", q)
        print("STATUS:", result["status"], "ROUTE:", result["route"], "INTENT:", result["detected_intent"])
