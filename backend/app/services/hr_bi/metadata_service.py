...
METADATA_FILE_SPECS: tuple[MetadataFileSpec, ...] = (
    ...
    MetadataFileSpec(
        ...
        aliases=("HR_BI_Assistant_Evaluation.json", "Template_07_HR_BI_Assistant_Evaluation.json", "Template 07 - HR BI Assistant Evaluation.json"),
    ),
    MetadataFileSpec(
        key="access_policies",
        kind="yaml",
...
class MetadataService:
    ...
    def _read_metadata_file(self, path: Path, spec: MetadataFileSpec) -> JsonDict:
        ...
        try:
            if spec.kind == "json":
                ...
                if yaml is None:
                    raise MetadataParseError("PyYAML is not installed. Add PyYAML>=6.0.2 to backend/requirements.txt")
    ...
    def _build_indexes(self) -> None:
        self._columns_by_name = self._index_by(self._raw.get("data_dictionary", {}).get("columns", []), "name")
        self._intents_by_id = self._index_by(self._raw.get("intent_catalog", {}).get("intents", []), "intent_id")
        self._reports_by_id = self._index_by(self._raw.get("report_catalog", {}).get("reports", []), "report_id")
        self._templates_by_id = self._index_by((self._raw.get("sql_templates", {}).get("templates", []) or []) + (self._raw.get("sql_templates", {}).get("status_templates", []) or []), "template_id")
        self._template_aliases = {**FALLBACK_TEMPLATE_ALIASES, **(self._raw.get("sql_templates", {}).get("template_aliases", {}) or {})}
        self._kpis_by_id = self._index_by(self._raw.get("kpi_catalog", {}).get("kpis", []), "kpi_id")
        self._visuals_by_intent = self._index_by(self._raw.get("visualization_rules", {}).get("intent_visualization_map", []), "intent")
        self._visuals_by_report = self._index_by(self._raw.get("visualization_rules", {}).get("report_visualization_map", []), "report_id")
        self._semantic_concepts_by_id = self._index_by(self._raw.get("semantic_layer", {}).get("semantic_concepts", []), "concept_id")
        self._term_index = self._raw.get("semantic_layer", {}).get("term_index_for_semantic_mapper", {}) or {}
    ...
    def _cross_validate_references(self, *, strict: bool) -> list[str]:
        ...
        for intent_id, intent in self._intents_by_id.items():
            ...
            if template_id and self.resolve_sql_template_id(template_id) not in self._templates_by_id:
                warnings.append(f"Intent '{intent_id}' references missing sql_template_id '{template_id}'.")
            ...
            warnings.extend(self._missing_columns_warning("Intent", intent_id, intent.get("required_columns", [])))
        ...
        for report_id, report in self._reports_by_id.items():
            ...
            if report.get("route") == "SQL" and template_id and self.resolve_sql_template_id(template_id) not in self._templates_by_id:
                warnings.append(f"Report '{report_id}' references missing sql_template_id '{template_id}'.")
            ...
            warnings.extend(self._missing_columns_warning("Report", report_id, report.get("required_columns", [])))

        for template_id, template in self._templates_by_id.items():
            warnings.extend(self._missing_columns_warning("SQL template", template_id, template.get("required_columns", [])))
        ...
        for kpi_id, kpi in self._kpis_by_id.items():
            ...
            if kpi.get("route") == "SQL" and template_id and self.resolve_sql_template_id(template_id) not in self._templates_by_id:
                warnings.append(f"KPI '{kpi_id}' references missing sql_template_id '{template_id}'.")
            warnings.extend(self._missing_columns_warning("KPI", kpi_id, kpi.get("required_columns", [])))
    ...
    def get_main_view(self) -> JsonDict:
        view = self._raw.get("data_dictionary", {}).get("main_view") or self._raw.get("report_catalog", {}).get("main_view") or {}
    ...
    def find_semantic_matches(self, question: str, *, max_matches: int = 20) -> list[JsonDict]:
        ...
        for concept_id, concept in self._semantic_concepts_by_id.items():
            ...
            matched_terms = [term for term in terms if isinstance(term, str) and term and term in normalized]
            if matched_terms:
                matches.append(
                    {
        ...
        matches.sort(key=lambda item: (priority_rank.get(item.get("priority"), 99), -len(item.get("matched_terms", []))))
    ...
    def build_schema_context_for_prompt(self, *, include_allowed_values: bool = True, include_semantics: bool = True) -> str:
        ...
        view_name = main_view.get("name") or main_view.get("relation") or "hr_mvp.vw_hr_employee_analytics"
        ...
        for column in self.get_columns(include_restricted=True):
            ...
            permissions = column.get("permissions", {}) if isinstance(column.get("permissions"), dict) else {}
        ...
        if include_semantics:
            ...
            semantic_concepts = self._raw.get("semantic_layer", {}).get("semantic_concepts", []) or []
            for concept in semantic_concepts[:80]:
                ...
                if not terms or not maps_to:
                    continue
                mapped_column = maps_to.get("column") or maps_to.get("metric") or maps_to.get("route")
    ...
    def build_metadata_context_for_intent(self, intent_id: str) -> JsonDict:
        ...
        template = self.get_sql_template(str(intent.get("sql_template_id"))) if intent.get("sql_template_id") else None
    ...
    def get_min_group_size(self, default: int = 5) -> int:
        ...
        aggregation_rules = policies.get("aggregation_rules", {}) if isinstance(policies.get("aggregation_rules"), dict) else {}
    ...
    def get_sensitive_columns(self) -> list[str]:
        blocklist = self._raw.get("sql_validator_rules", {}).get("sensitive_columns_blocklist", []) or []
