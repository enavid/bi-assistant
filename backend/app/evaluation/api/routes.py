...
def build_orchestrator(model_name: str | None = None) -> Any:
    """Build a fresh orchestrator for an eval run via the single composition root.

    Delegates to :func:`app.dependencies.build_hr_bi_orchestrator` so the eval
    pipeline is wired identically to production (same LLM client, model and
    template-engine/controlled-dynamic/force-llm flags) and honours the
    ``model_name`` override. Imported through the module so it stays patchable.
    """
    from app import dependencies

    return dependencies.build_hr_bi_orchestrator(model_name=model_name)
