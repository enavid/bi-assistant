...
class DecisionRouter:
    ...
    def route(
        ...
        if terminal:
            terminal["warnings"] = list(dict.fromkeys((terminal.get("warnings") or []) + warnings))[
        ...
        if not intent_result:
            ...
            if semantic_terminal:
                semantic_terminal["warnings"] = list(
                    dict.fromkeys((semantic_terminal.get("warnings") or []) + warnings)
                )[: self.config.max_warnings]
        ...
        if (
            intent_route in {ROUTE_GAP, ROUTE_REJECT, ROUTE_CLARIFICATION}
            or intent_status in TERMINAL_STATUS_TO_ROUTE
        ):
            ...
            return self._decision(
                ...
                reason=str(
                    ...
                    or intent_result.get("gap_reason")
                    or intent_result.get("reject_reason")
                    or "Terminal intent decision."
                ),
        ...
        if catalog_route in {ROUTE_GAP, ROUTE_REJECT, ROUTE_CLARIFICATION}:
            ...
            return self._decision(
                ...
                status=status,
                reason=str(
                    (intent_meta or {}).get("gap_reason")
                    or (intent_meta or {}).get("reject_reason")
                    or "Metadata catalog routes this intent away from SQL."
                ),
                decision_source="intent_catalog.terminal",
                intent_id=str(intent_id) if intent_id else None,
                report_id=self._str_or_none(
                    intent_result.get("report_id") or (intent_meta or {}).get("report_id")
                ),
                confidence=confidence,
                output_type="status_message",
                recommended_visualization="status_message",
        ...
        required_columns = self._merge_lists(
            intent_result.get("required_columns"), (intent_meta or {}).get("required_columns")
        )
        missing_columns = self._missing_columns(required_columns, service)
        if missing_columns:
            warnings.append(
                "Some required columns are missing from data_dictionary: "
        ...
        if report_status in {"data_gap", "unsupported", "not_ready"}:
            return self._decision(
                ...
                status=STATUS_DATA_GAP,
                reason=str(
                    (report_meta or {}).get("gap_reason")
                    or "The related report is marked as Data Gap / not ready."
                ),
        ...
        if sql_template_id and sql_template_id.upper() in _STATUS_TEMPLATE_IDS:
            ...
            resolved_route = self._route_for_status_or_route(
                status=resolved_status, route=tpl_route
            )
            return self._decision(
        ...
        if policy_hints.get("deny_sql"):
            return self._decision(
                ...
                reason=str(
                    policy_hints.get("reason") or "User role is not allowed to run this request."
                ),
        ...
        output_type = self._str_or_none(
            intent_result.get("output_type") or (sql_template or {}).get("output_type")
        )
        recommended_visualization = self._str_or_none(
            intent_result.get("recommended_visualization")
    ...
    def _terminal_from_payload(
        ...
        *,
        ...
        if raw_status in {
            ...
            STATUS_OK,
            STATUS_VALID,
            STATUS_SUPPORTED.upper(),
            "SUPPORTED",
        } and raw_route not in {
            ROUTE_GAP,
            ROUTE_REJECT,
            ROUTE_CLARIFICATION,
        }:
        ...
        if raw_status in TERMINAL_STATUS_TO_ROUTE or raw_route in {
            ROUTE_GAP,
            ROUTE_REJECT,
            ROUTE_CLARIFICATION,
        }:
            ...
            return self._decision(
                ...
                reason=str(
                    ...
                    or f"Terminal decision from {source}."
                ),
                ...
                recommended_visualization=self._str_or_none(
                    payload.get("recommended_visualization")
                )
                or "status_message",
                expected_status_sql=self._status_sql(status, service),
                started=started,
                ...
                extra_metadata={
                    "source_payload_status": raw_status,
    ...
    def _decision(
        ...
        *,
        ...
        decision = RouteDecision(
            ...
            policy_hints=policy_hints
            or self._build_policy_hints(user_role=user_role, intent_result={}, service=service),
            metadata={
                "phase": self.config.phase_name,
                "main_view": self.config.main_view,
                "main_alias": self.config.main_alias,
    ...
    def _resolve_template_id(
        ...
        intent_meta: JsonDict | None,
        report_meta: JsonDict | None,
        service: Any,
    ) -> str | None:
    ...
    def _build_policy_hints(
        ...
        return {
            ...
            "status_output": route in {ROUTE_GAP, ROUTE_REJECT, ROUTE_CLARIFICATION}
            or output_type == "status_message",
        }
    ...
    def _normalize_status(self, status: str, route: str | None) -> str:
        ...
        if status in {"", STATUS_OK, "SUPPORTED", STATUS_SUPPORTED.upper()}:
            return ROUTE_TO_DEFAULT_STATUS.get(
                route, STATUS_VALID if route == ROUTE_SQL else STATUS_NEEDS_CLARIFICATION
            )
        if status == "VALIDATED":
            return STATUS_VALID
        return status
...
def get_router(
    *, reload: bool = False, metadata_service: Any | None = None, config: RouterConfig | None = None
) -> DecisionRouter:
