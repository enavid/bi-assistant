...
def test_domain_classifier_rejects_sales_question(metadata_service):
    result = DomainClassifier().classify("میزان فروش ماه گذشته چقدر بود؟", metadata=metadata_service)
    assert result["is_hr"] is False
    assert result["route"] == "REJECT"
...
def test_question_validator_accepts_valid_hr_question(metadata_service):
    result = QuestionValidator().validate("تعداد کل کارکنان چند نفر است؟", metadata=metadata_service)
...
def test_question_validator_blocks_phone_number_request(metadata_service):
    result = QuestionValidator().validate("شماره تماس کارکنان واحد فروش را بده", metadata=metadata_service)
    assert result["is_valid"] is False
